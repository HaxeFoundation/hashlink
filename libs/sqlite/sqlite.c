...
**/
typedef struct _database HL_NAME(database);
typedef struct _result HL_NAME(result);

struct _database {
	void (*finalize)( HL_NAME(database) * );
	sqlite3 *db;
	HL_NAME(result) *last;
};

struct _result {
	void (*finalize)(HL_NAME(result) * );
	HL_NAME(database) *db;
	int ncols;
	int count;
...
static void HL_NAME(finalize_request)(HL_NAME(result) *r, bool exc ) {
	r->first = 0;
	r->done = 1;
	if( r->ncols == 0 )
...
static void HL_NAME(finalize_result)(HL_NAME(result) *r ) {
...
**/
HL_PRIM void HL_NAME(close)( HL_NAME(database) *db ) {
	if (db->last != NULL)
...
static void HL_NAME(finalize_database)( HL_NAME(database) *db ) {
...
**/
HL_PRIM HL_NAME(database) *HL_NAME(connect)( vbyte *filename ) {
	HL_NAME(database) *db;
	sqlite3 *sqlite;
	if( sqlite3_open16(filename, &sqlite) != SQLITE_OK ) {
		HL_NAME(error)(sqlite, true);
	}
	db = (HL_NAME(database)*)hl_gc_alloc_finalizer(sizeof(HL_NAME(database)));
...
**/
HL_PRIM int HL_NAME(last_id)(HL_NAME(database) *db ) {
...
**/
HL_PRIM HL_NAME(result) *HL_NAME(request)(HL_NAME(database) *db, vbyte *sql ) {
	HL_NAME(result) *r;
	const char *tl;
	int i,j;

	r = (HL_NAME(result)*)hl_gc_alloc_finalizer(sizeof(HL_NAME(result)));
...
**/
HL_PRIM vdynamic *HL_NAME(result_get_length)( HL_NAME(result) *r ) {
	if( r->ncols != 0 )
		return NULL;
...
**/
HL_PRIM int HL_NAME(result_get_nfields)( HL_NAME(result) *r ) {
	return r->ncols;
}
...
**/
HL_PRIM varray *HL_NAME(result_get_fields)(HL_NAME(result) *r) {
...
**/
HL_PRIM varray *HL_NAME(result_next)( HL_NAME(result) *r ) {
	if( r->done )
		return NULL;
...
**/
HL_PRIM vbyte *HL_NAME(result_get)( HL_NAME(result) *r, int n ) {
	if (n < 0 || n >= r->ncols)
		return NULL;
	if( r->first )
...
**/
HL_PRIM vdynamic *HL_NAME(result_get_int)( HL_NAME(result) *r, int n) {
	if (n < 0 || n >= r->ncols)
		return NULL;
	if( r->first )
		HL_NAME(result_next)(r);
...
**/
HL_PRIM vdynamic *HL_NAME(result_get_float)( HL_NAME(result) *r, int n ) {
	if( n < 0 || n >= r->ncols )
		return NULL;
	if( r->first )
		HL_NAME(result_next)(r);
...
#define _CONNECTION _ABSTRACT(HL_NAME(database))
#define _RESULT _ABSTRACT(HL_NAME(result))
