...
DEFINE_PRIM(_I32, fs_read_with_cb, _LOOP _FS _FILE _BUF _U32 _I64 _BOOL);
...
DEFINE_PRIM(_I32, fs_write_with_cb, _LOOP _FS _FILE _BUF _U32 _I64 _BOOL);
...
DEFINE_PRIM(_DYN, fs_get_ptr, _FS);
...
DEFINE_PRIM(_DYN, handle_get_data, _HANDLE);

DEFINE_PRIM(_DYN, handle_set_data, _HANDLE _DYN);
...
DEFINE_PRIM(_LOOP, default_loop, _VOID);
...
DEFINE_PRIM(_U64, loop_size, _VOID);
...
DEFINE_PRIM(_VOID, walk_with_cb, _LOOP _DYN);

DEFINE_PRIM(_I32, loop_fork, _LOOP);

DEFINE_PRIM(_DYN, loop_get_data, _LOOP);

DEFINE_PRIM(_DYN, loop_set_data, _LOOP _DYN);
...
DEFINE_PRIM(_VOID, library_shutdown, _VOID);
...
DEFINE_PRIM(_I32, os_getpid, _VOID);

DEFINE_PRIM(_I32, os_getppid, _VOID);
...
DEFINE_PRIM(_I32, inet_ntop, _I32 _DYN _BYTES _U64);

DEFINE_PRIM(_I32, inet_pton, _I32 _BYTES _DYN);
...
DEFINE_PRIM(_I64, get_free_memory, _VOID);

DEFINE_PRIM(_I64, get_total_memory, _VOID);

DEFINE_PRIM(_I64, get_constrained_memory, _VOID);

DEFINE_PRIM(_I64, hrtime, _VOID);
...
DEFINE_PRIM(_I32, random_with_cb, _LOOP _RANDOM _DYN _U64 _U32);
...
DEFINE_PRIM(_I32, pipe, _FILE _I32 _I32);
...
DEFINE_PRIM(_VOID, disable_stdio_inheritance, _VOID);
...
DEFINE_PRIM(_DYN, req_get_data, _REQ);

DEFINE_PRIM(_DYN, req_set_data, _REQ _DYN);
...
DEFINE_PRIM(_I32, write_with_cb, _WRITE _STREAM _BUF _U32);
...
DEFINE_PRIM(_I32, write2_with_cb, _WRITE _STREAM _BUF _U32 _STREAM);

DEFINE_PRIM(_I32, try_write, _STREAM _BUF _U32);

DEFINE_PRIM(_I32, try_write2, _STREAM _BUF _U32 _STREAM);
...
DEFINE_PRIM(_I32, socketpair, _I32 _I32 _OS_SOCK_T _I32 _I32);
...
DEFINE_PRIM(_I32, tty_reset_mode, _VOID);
...
DEFINE_PRIM(_U32, version, _VOID);

DEFINE_PRIM(_BYTES, version_string, _VOID);
