...
BREW_OPUSFILE_PREFIX := $(shell brew --prefix opusfile)
BREW_OPUS_PREFIX := $(shell brew --prefix opus)
CFLAGS += -m$(MARCH) -I include -I $(BREW_PREFIX)/include -I $(BREW_OPENAL_PREFIX)/include -I $(BREW_SDL_PREFIX)/include/SDL2 -I $(BREW_OPUSFILE_PREFIX)/include/opus -I $(BREW_OPUS_PREFIX)/include/opus -Dopenal_soft -DGL_SILENCE_DEPRECATION
...
LIBFLAGS += -L$(BREW_PREFIX)/lib -L$(BREW_OPENAL_PREFIX)/lib -L$(BREW_OPUSFILE_PREFIX)/lib -L$(BREW_OPUS_PREFIX)/lib
...
# Linux
CFLAGS += -m$(MARCH) -fPIC -pthread -fno-omit-frame-pointer $(shell pkg-config --cflags sdl2) $(shell pkg-config --cflags opusfile)
...
fmt: ${FMT} libhl
	${CC} ${CFLAGS} -shared -o fmt.hdll ${FMT} ${LIBFLAGS} -L. -lhl -lpng $(LIBTURBOJPEG) -lz -lvorbisfile -lopusfile -lopus -logg
